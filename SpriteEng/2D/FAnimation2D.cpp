...
FAnimation2D::FAnimation2D( const FString & sName ) : FGraphObject( sName, FGraphObjectManager::OBJECT_ANIMATION ), aFrameArr( 128 ), iAtlasBlock( 0 )
{
	...
	try
	{
		...
		for( UI32 i = 0;i < lpAtlasRes->GetFrameCount();i++ )
		{
			...
			lpFrame->iWidth = fAtlasFrame.iWidth;
			lpFrame->iHeight = fAtlasFrame.iHeight;
...
UI32 FAnimation2D::GetWidth( UI32 iFrame )const
{
	...
	return aFrameArr[iFrame]->iWidth;
}

UI32 FAnimation2D::GetHeight( UI32 iFrame )const
{
	if( iFrame >= aFrameArr.GetCount() )
	{
		FLog::PutError( "Frame index is out of maximum index %i, max %i", iFrame, aFrameArr.GetCount() );
	...
	return aFrameArr[iFrame]->iHeight;
}
...
void FAnimation2D::Draw( const FVector2F & vPos, UI32 iFrameIndex, F32 fAlpha, void * lpData, UI32 * lpIndeces, UI32 iStartVert )
{
	...
	//#1
	lpVertData->cColor = cColor;
	lpVertData->vPoint = FVector2F( vPos.x + lpFrame->iWidth, vPos.y );
	lpVertData->vTexCoord = FVector2F( lpFrame->u1, lpFrame->v1 );
	lpVertData++;
	//#2
	lpVertData->cColor = cColor;
	lpVertData->vPoint = FVector2F( vPos.x + lpFrame->iWidth, vPos.y + lpFrame->iHeight );
	lpVertData->vTexCoord = FVector2F( lpFrame->u1, lpFrame->v0 );
	lpVertData++;
	//#3
	lpVertData->cColor = cColor;
	lpVertData->vPoint = FVector2F( vPos.x, vPos.y + lpFrame->iHeight );
